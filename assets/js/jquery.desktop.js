...
var JQD = (function ($, window, document, undefined) {
	...
	return {
		...
		go: function (settings) {
			JQD.settings = jQuery.extend({
				wallpaper: 'assets/images/gui/wallpaper.jpg',
				clock24h: true
			}, settings);
		...
		init: {
			...
			desktop: function () {
				...
				d.mousedown(function (ev) {
					...
					// Special treatment for applications (application launches)
					if ($(ev.target).hasClass('application'))
						JQD.util.launch_app($(ev.target));
				...
				d.on('mousedown', 'a.menu_trigger', function (ev) {
					JQD.util.clear_active();
			...
			wallpaper: function () {
				...
				if ($('#desktop').length) {
					$('body').prepend('<img id="wallpaper" class="abs" src="' + JQD.settings['wallpaper'] + '" />');
				}
			}
		},
		util: {

			launch_app: function (link) {
				// Get the link's target.
				...
				// See if we have an iframe and a url for it?
				var iframe = $(y).find('iframe');
